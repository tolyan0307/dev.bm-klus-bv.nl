...
function PropMeter({ level, label }: { level: PropLevel; label: string }) {
  return (
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl max-h-[90dvh] overflow-y-auto">
        ...
        <div className="px-6 py-6">
          ...
          <div className="mb-6 grid grid-cols-3 gap-3">
            {(Object.entries(f.props) as [keyof typeof f.props, PropLevel][]).map(
              ([key, val]) => (
      ...
      <div className="flex flex-1 flex-col px-5 py-5">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            aria-label={
              isComparing
                ? `${f.name} uit vergelijking verwijderen`
                : `${f.name} toevoegen aan vergelijking`
            }
...
export default function GevelAfwerkingGidsInteractive({ finishes }: { finishes: FinishItem[] }) {
