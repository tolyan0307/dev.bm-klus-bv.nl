import { Fragment } from "react"
import Link from "next/link"
import { ArrowRight, ChevronRight, CheckCircle2 } from "lucide-react"
import { buildPageMetadata } from "@/lib/seo/meta"
import { jsonLdScript, projectPageSchema } from "@/lib/seo/schema"
import { SITE } from "@/lib/seo/routes"
import ProjectGalleryCarousel from "@/components/sections/projects/ProjectGalleryCarousel"
import WerkzaamhedenAccordion from "@/components/sections/projects/WerkzaamhedenAccordion"
import ResponsiveImage from "@/components/responsive-image"
import { beforeImages, afterImages } from "@/lib/content/projects/delft-willemstraat-gevelrenovatie-schilderwerk-2026"
import { resolveGalleryImages } from "@/lib/gallery-utils"

// ─── SEO ──────────────────────────────────────────────────────────────────────
export const metadata = buildPageMetadata(
  "/onze-werken/delft-willemstraat-gevelrenovatie-schilderwerk-2026/",
  {
    title: "Delft gevelherstel & schilderwerk – 2026",
    description:
      "Gevelrenovatie in Delft Willemstraat met pleisterherstel, dubbele wapening, schilderwerk in RAL 9001, nieuwe raamdorpels en Keralit.",
  },
)

// ─── Data ─────────────────────────────────────────────────────────────────────
const heroBullets = [
  "Lokaal herstel van beschadigd cementpleister met zes metalen ankers",
  "Zij- en achtergevel dubbel gewapend en geschilderd in RAL 9001",
  "Nieuwe raamdorpels, loodafwerking en donkere Keralit geveldelen",
]

const werkzaamheden = [
  {
    title: "Beschadigd cementpleister verwijderen en ondergrond herstellen",
    body: "Op de achtergevel zijn sterk beschadigde zones teruggebracht tot op het metselwerk. Voor extra stabiliteit zijn zes metalen ankers aangebracht, met primer behandeld en opnieuw ingestuct.",
  },
  {
    title: "Hoek- en stopprofielen monteren",
    body: "Rond ramen, deuren en woninghoeken zijn hoekprofielen geplaatst. Op de aansluiting tussen metselwerk en pleisterwerk is een stopprofiel gemonteerd voor een rechte en gecontroleerde beëindiging.",
  },
  {
    title: "Zij- en achtergevel dubbel wapenen",
    body: "De behandelde gevelvlakken zijn voorzien van twee lagen wapeningsnet om de nieuwe afwerkopbouw te versterken en spanningen beter over het oppervlak te verdelen.",
  },
  {
    title: "Oude verf schuren en gevels schilderen",
    body: "Op de achtergevel is de oude verflaag geschuurd. Na het herstel en de wapening zijn de zij- en achtergevel afgewerkt met gevelverf in RAL 9001.",
  },
  {
    title: "Raamdorpels en loodafwerking vernieuwen",
    body: "Twee bestaande betonnen vensterbanken zijn verwijderd en vervangen door nieuwe raamdorpels. Ook het oude lood boven het glazen dak is verwijderd en opnieuw aangebracht.",
  },
  {
    title: "Keralit geveldelen vervangen",
    body: "De oude Keralit bekleding aan de linker- en rechterzijde is gedemonteerd en vervangen door nieuwe donkere Keralit geveldelen, inclusief een verzorgde aansluiting rond ramen en dakranden.",
  },
]

const detailCards = [
  {
    title: "Zes metalen ankers in beschadigde zones",
    body: "De plaatselijk vrijgelegde metselwerkzones zijn met zes metalen ankers gestabiliseerd; de ankers zijn gegrond en vervolgens in de nieuwe pleisterlaag opgenomen.",
  },
  {
    title: "Wapeningsnet in twee lagen",
    body: "Zowel de zij- als achtergevel is dubbel gewapend als versterkte basis voor de nieuwe pleister- en schilderafwerking.",
  },
  {
    title: "Profielen rond openingen en overgangen",
    body: "Hoekprofielen rond ramen, deuren en woninghoeken en een stopprofiel bij de overgang naar metselwerk zorgen voor rechte randen en nette beëindigingen.",
  },
  {
    title: "Vernieuwde lood- en Keralit aansluitingen",
    body: "Het lood boven het glazen dak en de Keralit delen aan beide zijden zijn vernieuwd en zorgvuldig aangesloten op de omliggende gevelonderdelen.",
  },
]

const materialen = [
  { label: "Pleisterherstel", value: "Cementpleister voor lokaal herstel tot op het metselwerk" },
  { label: "Verankering", value: "Zes metalen ankers, behandeld met primer" },
  { label: "Wapening", value: "Wapeningsnet in twee lagen" },
  { label: "Profielen", value: "Hoekprofielen en stopprofiel voor pleisteraansluitingen" },
  { label: "Gevelafwerking", value: "Gevelverf in RAL 9001" },
  { label: "Raamdorpels", value: "Twee vernieuwde raamdorpels" },
  { label: "Loodwerk", value: "Vernieuwd lood boven het glazen dak" },
  { label: "Gevelbekleding", value: "Nieuwe donkere Keralit geveldelen" },
]

const bevindingen = [
  {
    title: "Beschadigd en plaatselijk los cementpleister",
    body: "Op de achtergevel waren zones aanwezig waar het bestaande cementpleister sterk was beschadigd en tot op het metselwerk moest worden verwijderd.",
  },
  {
    title: "Scheuren en verzwakte aansluitingen",
    body: "Scheurvorming en plaatselijke beschadigingen maakten extra verankering en een nieuwe gewapende opbouw noodzakelijk.",
  },
  {
    title: "Verouderde verf- en profielafwerking",
    body: "De oude verflaag en aansluitingen rond ramen, deuren, hoeken en de overgang naar het metselwerk vroegen om voorbereiding en een strakkere profielafwerking.",
  },
  {
    title: "Dorpels, lood en gevelbekleding aan vervanging toe",
    body: "De betonnen vensterbanken, het lood boven het glazen dak en de bestaande Keralit delen sloten niet meer aan op de gewenste vernieuwde gevelafwerking.",
  },
]

const resultaten = [
  {
    title: "Herstelde en versterkte gevelvlakken",
    body: "Beschadigde zones zijn opnieuw opgebouwd en de zij- en achtergevel hebben een doorlopende versterkte afwerklaag met twee lagen wapeningsnet gekregen.",
  },
  {
    title: "Egaal schilderwerk in RAL 9001",
    body: "De lichte schilderafwerking geeft de woning een rustiger en samenhangend gevelbeeld en laat de vernieuwde detaillering duidelijk uitkomen.",
  },
  {
    title: "Verzorgde openingen en aansluitingen",
    body: "Nieuwe profielen, raamdorpels en lood zorgen voor strakkere lijnen en nettere aansluitingen rond ramen, deuren, hoeken en het glazen dak.",
  },
  {
    title: "Nieuwe donkere Keralit gevelbekleding",
    body: "De donkere Keralit delen aan beide zijden vormen een strak contrast met het lichte schilderwerk en sluiten verzorgd aan op ramen en dakranden.",
  },
]

const relatedLinks = [
  { label: "Buiten stucwerk", href: "/buiten-stucwerk/" },
  { label: "Gevel schilderen", href: "/gevel-schilderen/" },
]

// ─── Page ─────────────────────────────────────────────────────────────────────
export default function DelftWillemstraatProjectPage() {
  return (
    <>
      {projectPageSchema({
        title: "Delft gevelherstel & schilderwerk – 2026",
        description:
          "Gevelrenovatie in Delft Willemstraat met pleisterherstel, dubbele wapening, schilderwerk in RAL 9001, nieuwe raamdorpels en Keralit.",
        url: `${SITE.canonicalBase}/onze-werken/delft-willemstraat-gevelrenovatie-schilderwerk-2026/`,
        image: "/images/projects/delft-willemstraat-gevelrenovatie-schilderwerk-2026/delft-willemstraat-gevelrenovatie-schilderwerk-2026-na-01.webp",
        city: "Delft (Willemstraat)",
        year: 2026,
        serviceTypes: ["Buiten-stucwerk", "Gevel schilderen"],
      }).map((s, i) => (
        <Fragment key={i}>{jsonLdScript(s)}</Fragment>
      ))}
      <div className="min-h-screen bg-background">

      {/* ── A · HERO ─────────────────────────────────────────────────────── */}
      <section className="relative isolate overflow-hidden min-h-[80vh] lg:min-h-[65vh]">

        {/* Background image */}
      <ResponsiveImage
        baseName="delft-willemstraat-gevelrenovatie-schilderwerk-2026-na-01"
        dir="/images/projects/delft-willemstraat-gevelrenovatie-schilderwerk-2026"
        preset="hero"
        alt=""
        aria-hidden="true"
        className="absolute inset-0 h-full w-full object-cover object-center"
        sizes="100vw"
        priority
        draggable={false}
      />

        {/* Layer 1 — base darkening */}
        <div className="absolute inset-0" style={{ background: "rgba(10,7,3,0.48)" }} />

        {/* Layer 2 — left-heavy content gradient */}
        <div
          className="absolute inset-0"
          style={{
            background:
              "linear-gradient(108deg, rgba(10,7,3,0.97) 0%, rgba(10,7,3,0.88) 22%, rgba(10,7,3,0.65) 44%, rgba(10,7,3,0.25) 64%, transparent 84%)",
          }}
        />

        {/* Layer 3 — top band for transparent navbar / white text */}
        <div
          className="absolute inset-x-0 top-0"
          style={{
            height: 200,
            background:
              "linear-gradient(to bottom, rgba(6,4,1,0.92) 0%, rgba(6,4,1,0.65) 38%, transparent 100%)",
          }}
        />

        {/* Layer 4 — orange brand glow bottom-left */}
        <div
          className="absolute rounded-full pointer-events-none"
          style={{
            bottom: "12%",
            left: "-6%",
            width: 480,
            height: 480,
            background: "radial-gradient(circle, rgba(232,96,10,0.16) 0%, transparent 68%)",
          }}
        />

        {/* Layer 5 — bottom fade into page */}
        <div
          className="absolute inset-x-0 bottom-0"
          style={{
            height: 80,
            background:
              "linear-gradient(to top, #ffffff 0%, rgba(255,255,255,0.70) 40%, transparent 100%)",
          }}
        />

        {/* Layer 6 — right vignette */}
        <div
          className="absolute inset-y-0 right-0"
          style={{
            width: "32%",
            background: "linear-gradient(to left, rgba(10,7,3,0.30), transparent)",
          }}
        />

        {/* Content */}
        <div className="relative z-10 container-default flex flex-col justify-between min-h-[80vh] lg:min-h-[65vh]">

          {/* Breadcrumbs */}
          <nav aria-label="Breadcrumb" className="pt-28 sm:pt-32 lg:pt-36 mb-7">
            <ol className="flex flex-wrap items-center gap-1.5 text-sm" style={{ color: "rgba(255,255,255,0.65)" }}>
              <li>
                <Link href="/" className="transition-colors hover:text-white">
                  Home
                </Link>
              </li>
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
              <li>
                <Link href="/onze-werken/" className="transition-colors hover:text-white">
                  Onze werken
                </Link>
              </li>
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
              <li className="font-medium text-white" aria-current="page">
                Delft (2026)
              </li>
            </ol>
          </nav>

          <div className="pb-16 sm:pb-20 lg:pb-24">
          {/* Badge */}
          <div className="mb-5">
            <span
              className="inline-block rounded-full px-4 py-1 text-xs font-semibold uppercase tracking-wider text-white"
              style={{ background: "rgba(232,96,10,0.90)", border: "1px solid rgba(232,96,10,0.60)" }}
            >
              Buiten-stucwerk
            </span>
          </div>

          {/* H1 */}
          <h1 className="text-balance text-3xl font-bold tracking-tight text-white sm:text-4xl lg:text-5xl max-w-3xl">
            Delft Willemstraat: gevelherstel, schilderwerk &amp; Keralit (2026)
          </h1>

          {/* Subheading */}
          <p className="mt-4 text-lg leading-relaxed max-w-2xl" style={{ color: "rgba(255,255,255,0.78)" }}>
            Bij deze woning aan de Willemstraat in Delft zijn beschadigde delen van het cementpleister verwijderd en opnieuw opgebouwd. De zij- en achtergevel zijn voorzien van hoek- en stopprofielen, twee lagen wapeningsnet en schilderwerk in RAL 9001. Daarnaast zijn twee raamdorpels, het lood boven het glazen dak en de Keralit geveldelen aan beide zijden vernieuwd.
          </p>

          {/* Passport */}
          <div
            className="mt-6 inline-flex flex-wrap gap-x-2 gap-y-1 rounded-xl px-4 py-3 text-sm"
            style={{
              background: "rgba(255,255,255,0.10)",
              border: "1px solid rgba(255,255,255,0.18)",
              backdropFilter: "blur(8px)",
              color: "rgba(255,255,255,0.75)",
            }}
          >
            <span className="font-medium text-white">Delft (Willemstraat)</span>
            <span aria-hidden>·</span>
            <span>woning</span>
            <span aria-hidden>·</span>
            <span>gevelherstel</span>
            <span aria-hidden>·</span>
            <span>wapeningsnet in twee lagen</span>
            <span aria-hidden>·</span>
            <span>RAL 9001</span>
            <span aria-hidden>·</span>
            <span>Keralit gevelbekleding</span>
          </div>

          {/* Bullets */}
          <ul className="mt-6 grid gap-2 sm:grid-cols-2 max-w-2xl" role="list">
            {heroBullets.map((b) => (
              <li key={b} className="flex items-start gap-2.5 text-sm" style={{ color: "rgba(255,255,255,0.82)" }}>
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" aria-hidden />
                {b}
              </li>
            ))}
          </ul>

          {/* CTAs */}
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/contact/" className="btn-hero">
              Offerte aanvragen
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
              href="/onze-werken/"
              className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-white/5 px-6 py-3 text-sm font-medium text-white/80 backdrop-blur-sm transition-all hover:border-white/25 hover:bg-white/10"
            >
              Terug naar Onze werken
            </Link>
          </div>
        </div>
        </div>
      </section>

      <div className="below-fold">
      {/* ── B · OVERZICHT WERKZAAMHEDEN ──────────────────────────────────── */}
      <section className="section-spacing border-b border-border" aria-labelledby="werkzaamheden-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Werkzaamheden</span>
          </div>
          <h2 id="werkzaamheden-heading" className="section-title max-w-2xl">
            Wat hebben we <span className="text-primary">uitgevoerd?</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            De renovatie combineerde constructief gevelherstel met een nieuwe gewapende afwerklaag, schilderwerk en vernieuwing van raamdorpels, lood en gevelbekleding.
          </p>

          <WerkzaamhedenAccordion items={werkzaamheden} />
        </div>
      </section>

      {/* ── C · VOOR DE WERKEN ───────────────────────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="voor-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Beginsituatie</span>
          </div>
          <h2 id="voor-heading" className="section-title max-w-2xl">
            Voor de <span className="text-primary">werken</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            Voor de renovatie vertoonden de gevels plaatselijke schade, scheuren, verweerde afwerklagen en verouderde aansluitdetails. De fotoreeks toont ook de opeenvolgende herstel-, wapenings- en montagefasen.
          </p>

          {/* Diagnosis cards */}
          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {bevindingen.map((item) => (
              <li
                key={item.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{item.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{item.body}</p>
              </li>
            ))}
          </ul>

          {/* Gallery */}
          <div className="mt-12">
            <div className="mb-4 flex items-center gap-3">
              <span className="h-px flex-1 bg-border" aria-hidden />
              <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Fotodocumentatie beginsituatie
              </span>
              <span className="h-px flex-1 bg-border" aria-hidden />
            </div>
            <ProjectGalleryCarousel
              title="Voor de werken"
              variant="voor"
              images={resolveGalleryImages(beforeImages)}
            />
          </div>
        </div>
      </section>

      {/* ── D · NA DE WERKEN ─────────────────────────────────────────────── */}
      <section className="section-spacing border-b border-border" aria-labelledby="na-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Eindresultaat</span>
          </div>
          <h2 id="na-heading" className="section-title max-w-2xl">
            Na de <span className="text-primary">werken</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            Na afronding vormen de licht geschilderde gevels, vernieuwde aansluitdetails en donkere Keralit bekleding één verzorgd geheel, met behoud van het karakter van de woning.
          </p>

          {/* Result cards */}
          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {resultaten.map((item) => (
              <li
                key={item.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{item.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{item.body}</p>
              </li>
            ))}
          </ul>

          {/* Gallery */}
          <div className="mt-12">
            <div className="mb-4 flex items-center gap-3">
              <span className="h-px flex-1 bg-border" aria-hidden />
              <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Fotodocumentatie eindresultaat
              </span>
              <span className="h-px flex-1 bg-border" aria-hidden />
            </div>
            <ProjectGalleryCarousel
              title="Na de werken"
              variant="na"
              images={resolveGalleryImages(afterImages)}
            />
          </div>
        </div>
      </section>

      {/* ── E · DETAILS DIE HET VERSCHIL MAKEN ──────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="details-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Vakmanschap</span>
          </div>
          <h2 id="details-heading" className="section-title max-w-2xl">
            Details die het <span className="text-primary">verschil maken</span>
          </h2>

          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            De technische kwaliteit zit in de combinatie van lokale verankering, dubbele wapening, profielafwerking en vernieuwde aansluitingen rond gevel en dak.
          </p>

          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {detailCards.map((card) => (
              <li
                key={card.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{card.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{card.body}</p>
              </li>
            ))}
          </ul>
        </div>
      </section>

      {/* ── F · MATERIALEN & AFWERKING ───────────────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="materialen-heading">
        <div className="container-default max-w-3xl">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Materialen</span>
          </div>
          <h2 id="materialen-heading" className="section-title">
            Materialen &amp; <span className="text-primary">afwerking</span>
          </h2>
          <p className="mt-3 text-base leading-relaxed text-muted-foreground">
            Overzicht van de belangrijkste materialen en afwerkingsonderdelen voor gevelherstel, wapening, schilderwerk en aansluitdetails.
          </p>

          <dl className="mt-8 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            {materialen.map((m, i) => (
              <div
                key={m.label}
                className={`group flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-primary/5 ${i !== 0 ? "border-t border-border" : ""}`}
              >
                <dt className="w-36 shrink-0 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  {m.label}
                </dt>
                <span className="h-4 w-px shrink-0 bg-border" aria-hidden />
                <dd className="text-sm font-medium text-foreground">{m.value}</dd>
              </div>
            ))}
          </dl>

          <p className="mt-5 text-sm leading-relaxed text-muted-foreground rounded-xl border border-border bg-card px-5 py-3.5">
            <strong className="font-semibold text-foreground">Let op:</strong> Exacte systeemkeuze is
            afhankelijk van ondergrond en situatie. Tijdens de gratis inspectie beoordelen wij welk
            systeem het meest geschikt is voor uw woning.
          </p>
        </div>
      </section>

      {/* ── H · GERELATEERDE DIENSTEN ────────────────────────────────────── */}
      <aside className="border-t border-border bg-secondary/10 py-10" aria-label="Gerelateerde diensten">
        <div className="container-default">
          <p className="mb-5 text-xs font-bold uppercase tracking-[0.2em] text-primary">Gerelateerde diensten</p>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="group flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-5 py-4 transition-all hover:border-primary/40 hover:shadow-sm"
              >
                <span className="text-sm font-semibold text-foreground transition-colors group-hover:text-primary">
                  {link.label}
                </span>
                <ArrowRight size={14} className="shrink-0 text-border transition-colors group-hover:text-primary" />
              </Link>
            ))}
          </div>
        </div>
      </aside>
      </div>{/* end below-fold */}

    </div>
    </>
  )
}
