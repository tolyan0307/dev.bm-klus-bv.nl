import { Fragment } from "react"
import Link from "next/link"
import { ArrowRight, ChevronRight, CheckCircle2 } from "lucide-react"
import { buildPageMetadata } from "@/lib/seo/meta"
import { jsonLdScript, projectPageSchema } from "@/lib/seo/schema"
import { SITE } from "@/lib/seo/routes"
import ProjectGalleryCarousel from "@/components/sections/projects/ProjectGalleryCarousel"
import WerkzaamhedenAccordion from "@/components/sections/projects/WerkzaamhedenAccordion"
import ResponsiveImage from "@/components/responsive-image"
import { beforeImages, afterImages } from "@/lib/content/projects/strijen-schenkeldijk-gevelisolatie-sierpleister-2026"
import { resolveGalleryImages } from "@/lib/gallery-utils"

// ─── SEO ──────────────────────────────────────────────────────────────────────
export const metadata = buildPageMetadata(
  "/onze-werken/strijen-schenkeldijk-gevelisolatie-sierpleister-2026/",
  {
    title: "Strijen gevelisolatie & sierpleister – 2026",
    description:
      "Gevelrenovatie in Strijen aan de Schenkeldijk met 6 cm isolatie, GW-Plus, witte sierpleister 1,5 mm, nieuwe raamdorpels en dakrandafwerking.",
  },
)

// ─── Data ─────────────────────────────────────────────────────────────────────
const heroBullets = [
  "6 cm buitengevelisolatie met plugbevestiging en wapeningsnet",
  "Gevelherstel met cementgebonden GW-Plus mortel en profielafwerking",
  "Witte sierpleister 1,5 mm met vernieuwde dakranden en raamdorpels",
]

const werkzaamheden = [
  {
    title: "Dagkanten, openingen en gevelhoeken voorbereiden",
    body: "Aan de dagkanten van ramen en deuren en op de buitenhoeken van de woning zijn profielen gemonteerd als rechte en beschermde basis voor het verdere stuc- en afwerkwerk.",
  },
  {
    title: "Gevelvlakken met GW-Plus mortel herstellen",
    body: "De daarvoor bestemde wanden zijn met cementgebonden GW-Plus mortel uitgevlakt en hersteld, zodat een stabiele ondergrond ontstond voor de gewapende afwerklaag.",
  },
  {
    title: "Buitengevelisolatie van 6 cm aanbrengen",
    body: "Op de te isoleren geveldelen zijn isolatieplaten van 6 cm gemonteerd en aanvullend mechanisch bevestigd met pluggen.",
  },
  {
    title: "Wapeningsnet monteren en wanden egaliseren",
    body: "Over de voorbereide gevelvlakken en isolatie is wapeningsnet aangebracht. Daarna zijn de wanden geplamuurd en geëgaliseerd als basis voor de eindafwerking.",
  },
  {
    title: "Afwerken met witte sierpleister 1,5 mm",
    body: "De gevels zijn afgewerkt met standaard witte sierpleister met een korrel van 1,5 mm, waardoor de verschillende geveldelen één rustig en samenhangend oppervlak vormen.",
  },
  {
    title: "Dakrandplanken en raamdorpels vernieuwen",
    body: "Aan de dakrand zijn planken gemonteerd. Naden en scheuren zijn met een tweecomponentensamenstelling behandeld, waarna de delen zijn geschuurd, gegrond en in twee lagen gelakt. De oude vensterbanken zijn verwijderd en vervangen door nieuwe raamdorpels.",
  },
]

const detailCards = [
  {
    title: "Profielen aan dagkanten en buitenhoeken",
    body: "Profielen rond ramen, deuren en woninghoeken zorgen voor rechte randen en beschermen kwetsbare aansluitingen in de nieuwe gevelafwerking.",
  },
  {
    title: "Isolatie 6 cm met plugverankering",
    body: "De isolatieplaten zijn aanvullend mechanisch met pluggen bevestigd als stabiele basis voor het wapeningsnet en de sierpleister.",
  },
  {
    title: "GW-Plus en wapeningsnet als basislaag",
    body: "Cementgebonden GW-Plus mortel, wapeningsnet en een geëgaliseerde laag vormen samen de versterkte ondergrond onder de decoratieve afwerking.",
  },
  {
    title: "Tweecomponentenherstel van dakrandplanken",
    body: "Naden en scheuren in de dakrandplanken zijn behandeld voordat de delen werden geschuurd, gegrond en met twee laklagen afgewerkt.",
  },
]

const materialen = [
  { label: "Gevelisolatie", value: "Isolatieplaten 6 cm" },
  { label: "Bevestiging", value: "Mechanische bevestiging met pluggen" },
  { label: "Herstelmortel", value: "Cementgebonden GW-Plus mortel" },
  { label: "Wapening", value: "Wapeningsnet met egalisatie- en plamuurlaag" },
  { label: "Gevelafwerking", value: "Standaard witte sierpleister 1,5 mm" },
  { label: "Profielen", value: "Profielen voor dagkanten, openingen, hoeken en plint" },
  { label: "Dakrandafwerking", value: "Planken, tweecomponentensamenstelling, primer en twee laklagen" },
  { label: "Raamdorpels", value: "Nieuwe raamdorpels ter vervanging van de oude vensterbanken" },
]

const bevindingen = [
  {
    title: "Bakstenen geveldelen zonder nieuwe isolatieopbouw",
    body: "Voor de werkzaamheden waren delen van de woning nog uitgevoerd in zichtbaar metselwerk en ontbrak daar een nieuwe buitenopbouw met isolatie, wapening en sierpleister.",
  },
  {
    title: "Gevelvlakken vroegen om herstel en uitvlakking",
    body: "De bestaande ondergrond en aansluitingen rond ramen, deuren en hoeken moesten worden voorbereid en plaatselijk met cementgebonden mortel worden hersteld.",
  },
  {
    title: "Dakranddelen met naden en scheuren",
    body: "De dakrandplanken vroegen om montage en een volledige herstel- en lakopbouw, inclusief behandeling van naden en scheuren.",
  },
  {
    title: "Bestaande vensterbanken aan vervanging toe",
    body: "De oude vensterbanken sloten niet meer aan op de vernieuwde gevelopbouw en zijn daarom gedemonteerd en vervangen.",
  },
]

const resultaten = [
  {
    title: "Nieuwe geïsoleerde en gewapende gevelopbouw",
    body: "De behandelde delen hebben een vernieuwde opbouw gekregen met 6 cm isolatie, mechanische bevestiging en een gewapende basislaag.",
  },
  {
    title: "Egaal wit gevelbeeld",
    body: "De standaard witte sierpleister van 1,5 mm verbindt de verschillende aanbouwen en gevelvlakken tot één rustige en verzorgde uitstraling.",
  },
  {
    title: "Strakke details rond ramen en hoeken",
    body: "Profielen en nieuwe raamdorpels zorgen voor rechte lijnen en nette aansluitingen rond raam- en deuropeningen en buitenhoeken.",
  },
  {
    title: "Verzorgde donkere dakrandafwerking",
    body: "De gemonteerde, herstelde en in twee lagen gelakte dakrandplanken vormen een duidelijk contrast met de lichte gevelafwerking.",
  },
]

const relatedLinks = [
  { label: "Gevelisolatie", href: "/gevelisolatie/" },
  { label: "Buiten stucwerk", href: "/buiten-stucwerk/" },
  { label: "Sierpleister", href: "/sierpleister/" },
]

// ─── Page ─────────────────────────────────────────────────────────────────────
export default function StrijenSchenkeldijkProjectPage() {
  return (
    <>
      {projectPageSchema({
        title: "Strijen gevelisolatie & sierpleister – 2026",
        description:
          "Gevelrenovatie in Strijen aan de Schenkeldijk met 6 cm isolatie, GW-Plus, witte sierpleister 1,5 mm, nieuwe raamdorpels en dakrandafwerking.",
        url: `${SITE.canonicalBase}/onze-werken/strijen-schenkeldijk-gevelisolatie-sierpleister-2026/`,
        image: "/images/projects/strijen-schenkeldijk-gevelisolatie-sierpleister-2026/strijen-schenkeldijk-gevelisolatie-sierpleister-2026-na-01.webp",
        city: "Strijen (Schenkeldijk)",
        year: 2026,
        serviceTypes: ["Gevelisolatie", "Sierpleister", "Buiten-stucwerk"],
      }).map((s, i) => (
        <Fragment key={i}>{jsonLdScript(s)}</Fragment>
      ))}
      <div className="min-h-screen bg-background">

      {/* ── A · HERO ─────────────────────────────────────────────────────── */}
      <section className="relative isolate overflow-hidden min-h-[80vh] lg:min-h-[65vh]">

        {/* Background image */}
      <ResponsiveImage
        baseName="strijen-schenkeldijk-gevelisolatie-sierpleister-2026-na-01"
        dir="/images/projects/strijen-schenkeldijk-gevelisolatie-sierpleister-2026"
        preset="hero"
        alt=""
        aria-hidden="true"
        className="absolute inset-0 h-full w-full object-cover object-center"
        sizes="100vw"
        priority
        draggable={false}
      />

        {/* Layer 1 — base darkening */}
        <div className="absolute inset-0" style={{ background: "rgba(10,7,3,0.48)" }} />

        {/* Layer 2 — left-heavy content gradient */}
        <div
          className="absolute inset-0"
          style={{
            background:
              "linear-gradient(108deg, rgba(10,7,3,0.97) 0%, rgba(10,7,3,0.88) 22%, rgba(10,7,3,0.65) 44%, rgba(10,7,3,0.25) 64%, transparent 84%)",
          }}
        />

        {/* Layer 3 — top band for transparent navbar / white text */}
        <div
          className="absolute inset-x-0 top-0"
          style={{
            height: 200,
            background:
              "linear-gradient(to bottom, rgba(6,4,1,0.92) 0%, rgba(6,4,1,0.65) 38%, transparent 100%)",
          }}
        />

        {/* Layer 4 — orange brand glow bottom-left */}
        <div
          className="absolute rounded-full pointer-events-none"
          style={{
            bottom: "12%",
            left: "-6%",
            width: 480,
            height: 480,
            background: "radial-gradient(circle, rgba(232,96,10,0.16) 0%, transparent 68%)",
          }}
        />

        {/* Layer 5 — bottom fade into page */}
        <div
          className="absolute inset-x-0 bottom-0"
          style={{
            height: 80,
            background:
              "linear-gradient(to top, #ffffff 0%, rgba(255,255,255,0.70) 40%, transparent 100%)",
          }}
        />

        {/* Layer 6 — right vignette */}
        <div
          className="absolute inset-y-0 right-0"
          style={{
            width: "32%",
            background: "linear-gradient(to left, rgba(10,7,3,0.30), transparent)",
          }}
        />

        {/* Content */}
        <div className="relative z-10 container-default flex flex-col justify-between min-h-[80vh] lg:min-h-[65vh]">

          {/* Breadcrumbs */}
          <nav aria-label="Breadcrumb" className="pt-28 sm:pt-32 lg:pt-36 mb-7">
            <ol className="flex flex-wrap items-center gap-1.5 text-sm" style={{ color: "rgba(255,255,255,0.65)" }}>
              <li>
                <Link href="/" className="transition-colors hover:text-white">
                  Home
                </Link>
              </li>
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
              <li>
                <Link href="/onze-werken/" className="transition-colors hover:text-white">
                  Onze werken
                </Link>
              </li>
              <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
              <li className="font-medium text-white" aria-current="page">
                Strijen (2026)
              </li>
            </ol>
          </nav>

          <div className="pb-16 sm:pb-20 lg:pb-24">
          {/* Badge */}
          <div className="mb-5">
            <span
              className="inline-block rounded-full px-4 py-1 text-xs font-semibold uppercase tracking-wider text-white"
              style={{ background: "rgba(232,96,10,0.90)", border: "1px solid rgba(232,96,10,0.60)" }}
            >
              Gevelisolatie
            </span>
          </div>

          {/* H1 */}
          <h1 className="text-balance text-3xl font-bold tracking-tight text-white sm:text-4xl lg:text-5xl max-w-3xl">
            Strijen Schenkeldijk: gevelisolatie, sierpleister &amp; dakrandafwerking (2026)
          </h1>

          {/* Subheading */}
          <p className="mt-4 text-lg leading-relaxed max-w-2xl" style={{ color: "rgba(255,255,255,0.78)" }}>
            Bij deze woning aan de Schenkeldijk in Strijen zijn gevelvlakken hersteld met cementgebonden GW-Plus mortel en voorzien van profielen en wapeningsnet. Op de daarvoor bestemde delen is 6 cm buitengevelisolatie met plugbevestiging aangebracht. De gevels zijn afgewerkt met standaard witte sierpleister 1,5 mm. Daarnaast zijn dakrandplanken gemonteerd en gelakt en zijn de oude vensterbanken vervangen door nieuwe raamdorpels.
          </p>

          {/* Passport */}
          <div
            className="mt-6 inline-flex flex-wrap gap-x-2 gap-y-1 rounded-xl px-4 py-3 text-sm"
            style={{
              background: "rgba(255,255,255,0.10)",
              border: "1px solid rgba(255,255,255,0.18)",
              backdropFilter: "blur(8px)",
              color: "rgba(255,255,255,0.75)",
            }}
          >
            <span className="font-medium text-white">Strijen (Schenkeldijk)</span>
            <span aria-hidden>·</span>
            <span>woning</span>
            <span aria-hidden>·</span>
            <span>gevelisolatie 6 cm</span>
            <span aria-hidden>·</span>
            <span>GW-Plus mortel</span>
            <span aria-hidden>·</span>
            <span>sierpleister 1,5 mm</span>
            <span aria-hidden>·</span>
            <span>dakrandafwerking</span>
          </div>

          {/* Bullets */}
          <ul className="mt-6 grid gap-2 sm:grid-cols-2 max-w-2xl" role="list">
            {heroBullets.map((b) => (
              <li key={b} className="flex items-start gap-2.5 text-sm" style={{ color: "rgba(255,255,255,0.82)" }}>
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-primary" aria-hidden />
                {b}
              </li>
            ))}
          </ul>

          {/* CTAs */}
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/contact/" className="btn-hero">
              Offerte aanvragen
              <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
              href="/onze-werken/"
              className="inline-flex items-center gap-2 rounded-lg border border-white/15 bg-white/5 px-6 py-3 text-sm font-medium text-white/80 backdrop-blur-sm transition-all hover:border-white/25 hover:bg-white/10"
            >
              Terug naar Onze werken
            </Link>
          </div>
        </div>
        </div>
      </section>

      <div className="below-fold">
      {/* ── B · OVERZICHT WERKZAAMHEDEN ──────────────────────────────────── */}
      <section className="section-spacing border-b border-border" aria-labelledby="werkzaamheden-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Werkzaamheden</span>
          </div>
          <h2 id="werkzaamheden-heading" className="section-title max-w-2xl">
            Wat hebben we <span className="text-primary">uitgevoerd?</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            De woning is als één gevelrenovatie aangepakt: van ondergrondherstel, profielen en isolatie tot wapening, sierpleister en de afwerking van dakranden en raamdorpels.
          </p>

          <WerkzaamhedenAccordion items={werkzaamheden} />
        </div>
      </section>

      {/* ── C · VOOR DE WERKEN ───────────────────────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="voor-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Beginsituatie</span>
          </div>
          <h2 id="voor-heading" className="section-title max-w-2xl">
            Voor de <span className="text-primary">werken</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            De beginsituatie bestond uit verschillende bakstenen en reeds voorbereide gevelvlakken, oude vensterbanken en dakranden die nog moesten worden afgewerkt. De reeks toont ook tussenfasen van de uitvoering.
          </p>

          {/* Diagnosis cards */}
          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {bevindingen.map((item) => (
              <li
                key={item.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{item.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{item.body}</p>
              </li>
            ))}
          </ul>

          {/* Gallery */}
          <div className="mt-12">
            <div className="mb-4 flex items-center gap-3">
              <span className="h-px flex-1 bg-border" aria-hidden />
              <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Fotodocumentatie beginsituatie
              </span>
              <span className="h-px flex-1 bg-border" aria-hidden />
            </div>
            <ProjectGalleryCarousel
              title="Voor de werken"
              variant="voor"
              images={resolveGalleryImages(beforeImages)}
            />
          </div>
        </div>
      </section>

      {/* ── D · NA DE WERKEN ─────────────────────────────────────────────── */}
      <section className="section-spacing border-b border-border" aria-labelledby="na-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Eindresultaat</span>
          </div>
          <h2 id="na-heading" className="section-title max-w-2xl">
            Na de <span className="text-primary">werken</span>
          </h2>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            Na afronding heeft de woning een samenhangend wit gevelbeeld met vernieuwde isolatie- en pleisterlagen, strakke raamdorpels en zorgvuldig afgewerkte donkere dakranden.
          </p>

          {/* Result cards */}
          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {resultaten.map((item) => (
              <li
                key={item.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{item.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{item.body}</p>
              </li>
            ))}
          </ul>

          {/* Gallery */}
          <div className="mt-12">
            <div className="mb-4 flex items-center gap-3">
              <span className="h-px flex-1 bg-border" aria-hidden />
              <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Fotodocumentatie eindresultaat
              </span>
              <span className="h-px flex-1 bg-border" aria-hidden />
            </div>
            <ProjectGalleryCarousel
              title="Na de werken"
              variant="na"
              images={resolveGalleryImages(afterImages)}
            />
          </div>
        </div>
      </section>

      {/* ── E · DETAILS DIE HET VERSCHIL MAKEN ──────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="details-heading">
        <div className="container-default">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Vakmanschap</span>
          </div>
          <h2 id="details-heading" className="section-title max-w-2xl">
            Details die het <span className="text-primary">verschil maken</span>
          </h2>

          <p className="mt-3 max-w-2xl text-base leading-relaxed text-muted-foreground">
            De kwaliteit van de renovatie zit in de combinatie van mechanisch bevestigde isolatie, gewapend gevelherstel, profielafwerking en zorgvuldig behandelde dakranddetails.
          </p>

          <ul className="mt-7 grid gap-3 sm:grid-cols-2" role="list">
            {detailCards.map((card) => (
              <li
                key={card.title}
                className="group relative flex flex-col gap-1.5 rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md overflow-hidden"
              >
                <span
                  className="absolute left-0 inset-y-0 w-[3px] rounded-l-xl bg-primary opacity-0 transition-opacity group-hover:opacity-100"
                  aria-hidden
                />
                <span
                  className="absolute left-0 top-5 w-[3px] h-4 rounded-r-full bg-primary/30"
                  aria-hidden
                />
                <h3 className="text-sm font-semibold leading-snug text-foreground">{card.title}</h3>
                <p className="text-xs leading-relaxed text-muted-foreground">{card.body}</p>
              </li>
            ))}
          </ul>
        </div>
      </section>

      {/* ── F · MATERIALEN & AFWERKING ───────────────────────────────────── */}
      <section className="section-spacing border-b border-border bg-secondary/30" aria-labelledby="materialen-heading">
        <div className="container-default max-w-3xl">
          <div className="section-header">
            <span className="section-header-line" aria-hidden />
            <span className="section-header-label">Materialen</span>
          </div>
          <h2 id="materialen-heading" className="section-title">
            Materialen &amp; <span className="text-primary">afwerking</span>
          </h2>
          <p className="mt-3 text-base leading-relaxed text-muted-foreground">
            Overzicht van de belangrijkste materialen en afwerkingsonderdelen voor isolatie, gevelherstel, sierpleister, dakranden en raamdorpels.
          </p>

          <dl className="mt-8 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            {materialen.map((m, i) => (
              <div
                key={m.label}
                className={`group flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-primary/5 ${i !== 0 ? "border-t border-border" : ""}`}
              >
                <dt className="w-36 shrink-0 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  {m.label}
                </dt>
                <span className="h-4 w-px shrink-0 bg-border" aria-hidden />
                <dd className="text-sm font-medium text-foreground">{m.value}</dd>
              </div>
            ))}
          </dl>

          <p className="mt-5 text-sm leading-relaxed text-muted-foreground rounded-xl border border-border bg-card px-5 py-3.5">
            <strong className="font-semibold text-foreground">Let op:</strong> Exacte systeemkeuze is
            afhankelijk van ondergrond en situatie. Tijdens de gratis inspectie beoordelen wij welk
            systeem het meest geschikt is voor uw woning.
          </p>
        </div>
      </section>

      {/* ── H · GERELATEERDE DIENSTEN ────────────────────────────────────── */}
      <aside className="border-t border-border bg-secondary/10 py-10" aria-label="Gerelateerde diensten">
        <div className="container-default">
          <p className="mb-5 text-xs font-bold uppercase tracking-[0.2em] text-primary">Gerelateerde diensten</p>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="group flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-5 py-4 transition-all hover:border-primary/40 hover:shadow-sm"
              >
                <span className="text-sm font-semibold text-foreground transition-colors group-hover:text-primary">
                  {link.label}
                </span>
                <ArrowRight size={14} className="shrink-0 text-border transition-colors group-hover:text-primary" />
              </Link>
            ))}
          </div>
        </div>
      </aside>
      </div>{/* end below-fold */}

    </div>
    </>
  )
}
